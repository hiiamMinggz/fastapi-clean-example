...
class WalletService:
    ...
    def credit(self, wallet: Wallet, amount: Balance) -> None:
        """credits the wallet with the specified amount"""
        if amount.value <= 0:
            raise DomainError("Amount must be positive")
        wallet.balance += amount
        
    def debit(self, wallet: Wallet, amount: Balance) -> None:
    ...
    def transfer(self, from_wallet: Wallet, to_wallet: Wallet, amount: Balance) -> None:
